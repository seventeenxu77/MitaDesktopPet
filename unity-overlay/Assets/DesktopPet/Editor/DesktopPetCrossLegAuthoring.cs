...
namespace DesktopPetEditor
{
    public static class DesktopPetCrossLegAuthoring
    {
        ...
        public const float EnterDuration = 0.95f, HoldDuration = 5f, ExitDuration = 0.95f;
        public const float Duration = EnterDuration + HoldDuration + ExitDuration;
        ...
        public static void Generate()
        {
            ...
            using (var view = new DesktopPetDragReview.ReviewScene())
            {
                ...
                var clip = new AnimationClip { name = "SitCrossLeg", frameRate = 30 };
        ...
        public static void ConfigureStates(AnimatorController controller)
        {
            ...
            foreach (var transition in sit.transitions.Where(t => t.destinationState != null &&
                (t.destinationState.name == "SitCrossLeg" || t.destinationState.name == "SitRest")).ToArray())
                sit.RemoveTransition(transition);
            foreach (var state in machine.states.Where(s => s.state.name == "SitCrossLeg" || s.state.name == "SitRest").ToArray())
                machine.RemoveState(state.state);
            var cross = machine.AddState("SitCrossLeg", new Vector3(1000, 180));
            var rest = machine.AddState("SitRest", new Vector3(1250, 180));
            cross.motion = AssetDatabase.LoadAssetAtPath<AnimationClip>(ClipPath);
            rest.motion = sit.motion;
            var start = sit.AddTransition(cross);
            start.hasExitTime = true; start.exitTime = 2f / ((AnimationClip)sit.motion).length;
            start.hasFixedDuration = true; start.duration = .06f;
            ...
            finish.hasExitTime = true; finish.exitTime = 1; finish.hasFixedDuration = true; finish.duration = .08f;
        ...
        public static void RenderReview()
        {
            ...
            using (var view = new DesktopPetDragReview.ReviewScene())
            {
                foreach (float t in new[] {0f, .3f, .6f, .95f, 1.5f, 2f, 2.5f, 3.5f, 4.5f, 5.8f, 6.3f, Duration})
                {
        ...
        public sealed class SeatedRig
        {
            ...
            public void Pose(float seconds)
            {
                ...
                float progress = seconds < EnterDuration ? seconds/EnterDuration : seconds > EnterDuration+HoldDuration ?
                    (Duration-seconds)/ExitDuration : 1;
                float blend = Ease(progress);
                if (blend <= 0) return;
                float crossing = Ease((progress-.22f)/.78f);
                float lift = Mathf.Sin(Mathf.PI * Mathf.Clamp01(progress));
                float phase = Mathf.Max(0, seconds-EnterDuration);
